...
//		curCloneKeys = refList ? node.getCloneList(true),
	...
	if( key != null && key !== this.key ) {
		if( keyMap[key] ) {
			$.error("[ext-clones] reRegister(" + key + "): already selectorExists: " + this);
		}
		// Update keyMap
...
//		ctx.tree.debug("clones.treeRegisterNode", add, node);
		...
		if( add ) {
			if( keyMap[node.key] != null ) {
				$.error("clones.treeRegisterNode: node.key already selectorExists: " + node);
			}
			keyMap[key] = node;
			if( refKey ) {
...
//							node.debug("clones.treeRegisterNode: last =>", node.getCloneList());
	...
	nodeRenderStatus: function(ctx) {
		...
		if( ctx.options.clones.highlightClones ) {
			...
			// Only if span already selectorExists
...
$.ui.fancytree.registerExtension({
	...
	treeCreate: function(ctx){
		...
		// Get/Set a unique Id on the container (if not already selectorExists)
